...
use crate::parser::ast::invoke::{parse_invoke, Invoke};
use crate::parser::ast::op::{parse_binary_op, BinaryOp};
    ...
    pratt_parser()
        .map_primary(|primary| {
            ...
            match primary.as_rule() {
                Rule::number => Spanned::new(Expression::NumberLiteral {
                    value: parse_spanned_t(primary),
                    typ: MaybeNull::alloc_unknown(),
                }, span.into()),
                Rule::boolean => Spanned::new(Expression::BooleanLiteral(parse_spanned_t(primary)), span.into()),
                Rule::identifier => Spanned::new(Expression::Variable {
                    name: parse_spanned_t(primary),
                    typ: MaybeNull::alloc_unknown(),
                }, span.into()),
                Rule::expression => parse_expression(primary),
                Rule::invoke => Spanned::new(Expression::Invoke(parse_invoke(primary)), span.into()),
                _ => unreachable!(),
            }
        })
        .map_infix(|lhs, op, rhs| {
            let span = lhs.span().to(rhs.span());
            Spanned::new(Expression::BinaryOp {
