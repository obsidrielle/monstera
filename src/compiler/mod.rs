...
macro_rules! build_int_compare {
    ($this:ident, $lhs:ident, $rhs:ident, $predicate:path) => {
        match ($lhs.get_type(), $rhs.get_type()) {
            (BasicTypeEnum::IntType(_), BasicTypeEnum::IntType(_)) => $this
                .ir_builder
...
macro_rules! build_int_arithmetic {
    ($this:ident, $lhs:ident, $rhs:ident, $method:ident) => {
        match ($lhs.get_type(), $rhs.get_type()) {
            (BasicTypeEnum::IntType(_), BasicTypeEnum::IntType(_)) => $this
                .ir_builder
                .$method($lhs.into_int_value(), $rhs.into_int_value(), "")
                .unwrap()
                .into(),
            _ => unreachable!(),
        }
...
impl<'ctx> Compiler<'ctx> {
    ...
    pub(crate) fn compile_program(&mut self, program: &Program) {
        for function in &program.functions {
            ...
            self.compile_function(function).expect("TODO: panic message");
        }
    }
    ...
    fn compile_if_statement(&mut self, stat: &Spanned<IfStatement>) {
        ...
        self.ir_builder.position_at_end(current_block);

        let condition = self
            .compile_expression(&if_block.condition)
            .into_int_value();
        self.ir_builder
            .build_conditional_branch(condition, then_block, first_else_dest)
            .unwrap();
        ...
        for (i, else_if) in else_if_block.iter().enumerate() {
            ...
            let then_block = self
                .context
                .append_basic_block(function, &format!("elseif.then.{}", i));

            let next_else_dest = if i < else_if_block.len() - 1 {
                self.context
                    .append_basic_block(function, &format!("elseif.then.{}", i + 1))
            } else if else_block.is_some() {
                self.context.append_basic_block(function, "else.final")
            } else {
                merge_block
            ...
            self.ir_builder
                .build_conditional_branch(condition, then_block, next_else_dest)
                .unwrap();
            self.ir_builder.position_at_end(then_block);
        ...
        if let Some(else_b) = else_block {
            self.ir_builder.position_at_end(current_cond_block);
            self.compile_block(&else_b.block);
    ...
    fn compile_statement(&mut self, statement: &Spanned<Statement>) {
        match &statement.node {
            ...
            Statement::If(stat) => self.compile_if_statement(stat),
    ...
    fn function_metadata<'a>(
        &mut self,
        function_ast: &'a Spanned<Function>,
    ) -> (
        String,
        Vec<String>,
    ...
    fn compile_function(&mut self, function_ast: &Spanned<Function>) -> anyhow::Result<()> {
        let (function_name, args_names, args_types, return_type, block) =
        ...
        self.with_new_function_scope(|this| {
            this.compile_function_params(function, args_names, args_types)
                .unwrap();
    ...
    fn compile_expression(&mut self, expression: &Expression) -> BasicValueEnum<'ctx> {
        match expression {
            ...
            Expression::BooleanLiteral(value) => self
                .context
                .bool_type()
                .const_int(value.node as u64, false)
                .into(),
            Expression::Variable { name, .. } => {
                let var = &self.symbols[&name.node];
                self.ir_builder
                    .build_load(var.typ, self.variable(name), "")
                    .unwrap()
            }
            ...
            Expression::Invoke(invoke) => {
                ...
                let call_site_value = self
                    .ir_builder
                    .build_call(function, &args, "tmp_call")
                    .unwrap();
            ...
            Expression::BinaryOp { op, lhs, rhs } => {
                ...
                match op {
                    ...
                    BinaryOp::GreaterOrEqual => {
                        build_int_compare!(self, lhs, rhs, IntPredicate::SGE)
                    }
    ...
    fn dtype_to_basic_llvm_type(&self, dtype: DType) -> BasicTypeEnum<'ctx> {
        use crate::parser::ast::IntType;
        match dtype {
            DType::Int(int) => match int {
                IntType::I8 => self.context.i8_type().into(),
                IntType::I16 => self.context.i16_type().into(),
                IntType::I32 => self.context.i32_type().into(),
                IntType::I64 => self.context.i64_type().into(),
                IntType::U8 => self.context.i8_type().into(),
                IntType::U16 => self.context.i16_type().into(),
                IntType::U32 => self.context.i32_type().into(),
                IntType::U64 => self.context.i64_type().into(),
            },
    ...
    fn int_dtype_to_int_llvm_type(&self, dtype: DType) -> IntType<'ctx> {
        self.dtype_to_basic_llvm_type(dtype).into_int_type()
    }
